...
int main() {
    ...
    for (Node* cur = start; cur != NULL; cur = cur->pNext) {
    if (butiful(cur->data) == 2) { // Проверяем, является ли число "красивым"
        Node* lastInserted = cur->pNext; // Указатель на последний вставленный узел
        for (int i = 0; i < 5; i++) { 
        ...
        cur->pNext;*/
        //cur = lastInserted;
        cur=lastInserted;// Переход к следующему элементу после всех вставленных.
    } else {
        //cur->pNext; // Переход к следующему элементу.
    }
}
